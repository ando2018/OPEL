...
export class VehicleBrochureComponent implements OnInit {
    ...
    getUrlQrCodePodium(url: string) {
        return `${environment.mediasBaseUrl}/podium/analytics-pdf/${this.vehicle.brochureName
            }/${btoa(this.vehicleSharedService.getNameFile(url))}`;

    }

    getUrlQrCodeStand(url: string) {
        return `${environment.mediasBaseUrl}/stand/analytics-pdf/${this.vehicle.brochureName
            }/${btoa(this.vehicleSharedService.getNameFile(url))}`;
