...
export class ModelService {
    ...
    partialUpdate(model: Partial<Model>): Observable<Model> {
        if (environment.isMock) {
            return this.http
                .get('http://localhost:4200/assets/mocks/_model.json')
                .pipe(map(res => res as Model));
        } else {
            return this.http
                .patch(`${environment.apiUrl}/_model/${model._id}`, model)
                .pipe(map(res => res as Model));
        }
    }
    ...
    getVehicleModels(vehicle: Vehicle): Observable<Model[]> {
        if (environment.isMock) {
            ...
            return this.http
                .get(`${environment.apiUrl}/_model/findByVehiculeId/${vehicle._id}`)
                .pipe(map(res => res as Model[]));
        }
    }
    ...
    getModelById(modelId: string): Observable<Model> {
        if (environment.isMock) {
            ...
            return this.http
                .get(`${environment.apiUrl}/_model/${modelId}`)
                .pipe(map(res => res as Model));
        }
    }

    deleteModel(modelId: string) {
        return this.http.delete(`${environment.apiUrl}/_model/${modelId}`);
    }
    ...
    createModel(model: Model) {
        ...
        if (environment.isMock) {
            ...
            return this.http
                .post(`${environment.apiUrl}/_model`, model)
                .pipe(map(res => res as Model));
        }
    }
