...
(function(global, factory) {
    ...
    if (typeof define === 'function' && define.amd) define((exported = keyLime));

    if (typeof exports === 'object') module.exports = exported = keyLime;
    ...
    var holdTimer,
        ...
        spKeys = {
            ...
            Shift: function(f) {
                ...
                if (t) t.classList.remove('lime-toggle');
                ...
                if (shift || caps) imeCtr.classList.add('shift-toggle');
            },
            ...
            Symbol: function(f) {
                ...
                if (sym) f.classList.toggle('lime-toggle');
            },
            ...
            Backspace: function() {
                ...
                // ...but fails for some elements in newer WebKit/Blink
                    ...
                    if (!String(sel).length) sel.modify('extend', 'backward', 'character');
            ...
            Caret: function(f) {
                ...
                if (move) f.classList.add('lime-toggle');
            }
        },
    ...
    for (var i = 0; i < cssRules.length; i++) style.sheet.insertRule(cssRules[i], i);
    ...
    function hideIME() {
        ...
        hideTimer = setTimeout(function() {
            if (!visible || !dispatchCustomEvent('keylimehide')) return;
    ...
    function initKeys() {
        var html = '',
            spKeys = {
                ...
                row3: {
                    before:
                        '<li id="limeShift" class="lime-key lime-special-key lime-shift">↑</li>',
            ...
            .map(function(keyRow, rIdx) {
                ...
                return (
                    ...
                    keyRow
                        .map(function(k, kIdx) {
                            return (
                                '<li class="lime-key" data-text="' +
                                k +
                                '" data-symbol="' +
                                keys.symbol[rIdx][kIdx] +
                                '"></li>'
                            );
                        })
                        .join('') +
    ...
    function newFocus(el) {
        if (!el.classList.contains('lime-key')) return;

        if (focused) focused.classList.remove('lime-focus');
    ...
    function moveKeyFocus(dir) {
        ...
        if (!focused) next = imeCtr.querySelector('.lime-key');
        ...
        // Up/down need to use elementFromPoint()
            ...
            if (!lastLeft) x = lastLeft = rect.left + focused.offsetWidth / 2;
            ...
            if (!next.classList.contains('lime-key')) next = document.elementFromPoint(x + 10, y);
        }
    ...
    function doKeyAction() {
        ...
        if ((sp = spKeys[k.id.slice(4)])) return sp(k);
        else {
            ...
            // For <input> types number, email, date, et al
                ...
                while (len-- > 0) s.modify('move', 'backward', 'character');
            }
            doPostInput(a);
        }
    }
    ...
    function submit() {
        ...
        if (f) {
            ...
            if (evtConstructors) e = new Event('submit', { bubbles: true, cancelable: true });
            else {
    ...
    function showDiacritics() {
        ...
        if (fRect.right + w <= rRect.right) menu.style.left = fRect.right - cRect.left + 'px';
        else menu.style.right = cRect.right - fRect.left + 'px';
    ...
    function dispatchCustomEvent(type) {
        ...
        // DOM4 constructor
        if (evtConstructors) evt = new CustomEvent(type, { bubbles: true, cancelable: true });
        // Older method
        else {
    ...
    document.addEventListener(
        ...
        function(evt) {
            ...
            if (
                !exports.config.noauto &&
                (evt.target.isContentEditable || tag === 'INPUT' || tag === 'TEXTAREA')
            )
                showIME();
    ...
    document.addEventListener(
        'blur',
        function() {
            if (visible && !exports.config.noauto) hideIME();
        },
        true
    );
        ...
        function(evt) {
            ...
            switch (key) {
                    ...
                    if (focused) {
                        if (!holdTimer)
                            holdTimer = setTimeout(
                                showDiacritics,
                                exports.config.keyHoldTimeout || 500
                            );
                ...
                case 'BrowserBack':
                    ...
                    if (move) spKeys.Caret(focused);
                    else if (diacriticsMenu) hideDiacritics();
                    else hideIME();
            }
        },
        true
    ...
    /**
     * Fan _service: flash the buttons as they are pressed on a hardware keyboard
     */
    document.addEventListener(
        'keypress',
        function(evt) {
            // Don't flash on password elements, exit if no char code
            if (evt.target.type === 'password' || evt.charCode === 0 || evt.keyCode === 13) return;
            ...
            switch (evt.key.replace(/^Arrow/, '')) {
                ...
                case 'Enter':
                    ...
                    if (diacriticsMenu && !focused.classList.contains('lime-diacritical'))
                        ...
                        if (diacriticsMenu) hideDiacritics();
                    }
            }
        },
        true
     ...
     */
    imeCtr.addEventListener(
        'mousedown',
        function(evt) {
            swallowEvt(evt);

            if (!evt.target.classList.contains('lime-key')) return;
    ...
    imeCtr.addEventListener('mouseout', function(evt) {
        if (focused === evt.target) focused.classList.remove('lime-focus');
    });
    ...
    (function() {
        ...
        if (
            global.KeyEvent &&
            global.KeyEvent.prototype &&
            !global.KeyEvent.prototype.hasOwnProperty('key')
        )
